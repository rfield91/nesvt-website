...
const Item = ({ title, children }: ItemProps) => {
    ...
    return <div className="border border-slate-500 rounded shadow">
        <div className={`flex gap-2 p-2 cursor-pointer select-none ${isOpen ? "border-b border-b-slate-500" : ""}`} onClick={() => setIsOpen(prev => !prev)}>
            <div className="w-3">{isOpen ? <>-</> : <>+</>}</div>
            <div>{title}</div>
        </div>
        <div className={`${isOpen ? "" : "hidden"}`}><div className="p-2">{children}</div></div>
    </div>
};
    ...
    const items = [
        {
            ...
            content: <div className="flex flex-col gap-4">
                <p>This video gives a very good over view of the sport of autocross and
                    is about 9 minutes long.</p>
                ...
                <p>Part of what makes autocross so inexpensive is the shared work load.
        ...
        },
        {
            ...
            content: <div className="flex flex-col gap-2">
                <p>
                    A typical course layout is 1.8 Miles long at takes around 50 to 80 seconds to complete.
                </p>
                <p>
        ...
        },
        {
            ...
            content: <div className="flex flex-col gap-2">
                <p>Yearly membership is $10 and is required in order to participate
                    in any NE-SVT event. </p>
                ...
                <div>
                    ...
                    >
                        Sign up for {new Date().getFullYear().toString()} NE-SVT membership
                    </a>
                </div>
            </div>
        }]
